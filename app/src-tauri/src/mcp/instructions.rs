//! Server `instructions` string returned by the MCP `initialize` method.
//!
//! Verbatim port of `Sources/PalmierPro/Agent/Tools/AgentInstructions.swift`'s
//! `serverInstructions` constant. Keeping this byte-identical to the Swift
//! version is a contract requirement: clients (Claude / Codex / Cursor) use
//! this string to steer their behavior, and any drift would silently change
//! the assistant's posture between the macOS and Windows ports.

/// The instructions string advertised to MCP clients on `initialize`.
